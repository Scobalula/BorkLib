...
namespace Borks.Compression.ZLIB
{
    public static unsafe class ZLIB
    {
        ...
        public static int CompressBound(int size) => ZLIBInterop.MZ_deflateBound(IntPtr.Zero, size);
        ...
        public static byte[] Decompress(byte[] inputBuffer, int outputSize, bool hasZlibHeader)
        {
            ...
            fixed (byte* a = &inputBuffer[0])
            {
                ...
                {
                    var result = (ZLIBInterop.MiniZReturnStatus)ZLIBInterop.MZ_uncompress(
                        b,
                        ref rawBufSize,
                        a,
                        inputBuffer.Length,
                    ...
                    if (result != ZLIBInterop.MiniZReturnStatus.OK)
                        throw new CompressionException($"Failed to decompress the data, error returned: {result}");
                }
            }
        ...
        public static byte[] Compress(byte[] inputBuffer)
        {
            ...
            fixed (byte* a = &inputBuffer[0])
            {
                ...
                fixed (byte* b = &temp[0])
                {
                    var result = (ZLIBInterop.MiniZReturnStatus)ZLIBInterop.MZ_compress(
                        b,
                    ...
                    if (result != ZLIBInterop.MiniZReturnStatus.OK)
                        throw new CompressionException($"Failed to compress the data, error returned: {result}");

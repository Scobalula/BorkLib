...
{
    /// <summary>
    /// Provides extension methods for <see cref="Stream"/> instances.
    /// </summary>
    public static class StreamExtensions
    {
        ...
        {
            return Scan(stream, BytePattern.ParseString(hexString), stream.Seek(-stream.Length, SeekOrigin.End), stream.Seek(0, SeekOrigin.End), false);
        }
        ...
        {
            return Scan(stream, BytePattern.ParseString(hexString), stream.Seek(-stream.Length, SeekOrigin.End), stream.Seek(0, SeekOrigin.End), firstOccurence);
        }
        ...
        {
            return Scan(stream, BytePattern.ParseString(hexString), startPosition, endPosition, false);
        }
        ...
        {
            return Scan(stream, BytePattern.ParseString(hexString), startPosition, endPosition, firstOccurence);
        }

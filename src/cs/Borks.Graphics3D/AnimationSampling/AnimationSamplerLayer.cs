...
namespace Borks.Graphics3D.AnimationSampling
{
    ...
    public class AnimationSamplerLayer
    {
        ...
        public void Update(float time, AnimationSampleType sampleType, bool loop, float frameRate)
        {
            if (sampleType == AnimationSampleType.DeltaTime)
            {
                // Delta time from prev sample
                CurrentTime += time * frameRate;
            }
            else if (sampleType == AnimationSampleType.AbsoluteTime)
            {
                CurrentTime = time * frameRate;
            }
            else
        ...
        public float GetWeight(float time, float startTime)
        {
            ...
            var (firstIndex, secondIndex) = AnimationHelper.GetFramePairIndex(Weights, time, startTime, cursor: CurrentWeightsCursor);
